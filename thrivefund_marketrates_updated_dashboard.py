...
def compute_roi(transactions, benchmark_changes):
    ...
    # Compound interest with maturity logic
    balance = 0
    total_profit = 0
    ...
    for i in range(len(transactions)):
        if i == 0:
            ...
            if days > 0:
                rate = transactions.loc[i-1, 'Customer Rate'] / 100
                balance *= (1 + rate / 365) ** days
            balance += transactions.loc[i, 'Amount']
        transactions.loc[i, 'Interest'] = balance - transactions['Amount'][:i+1].sum()
        transactions.loc[i, 'Profit'] = transactions.loc[i, 'Interest']
        total_profit = transactions.loc[i, 'Profit']
